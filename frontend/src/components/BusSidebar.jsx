/**
 * BusSidebar — Collapsible per-bus info cards.
 */
...
function getStatusLabel(bus) {
  ...
  if (bus.signal_strength < 40) return { label: 'Sparse', color: 'text-orange-500', dot: 'bg-orange-400' };
  return { label: 'Live', color: 'text-green-600', dot: 'bg-green-500' };
}

const ROUTE_COLORS = {
  route_purple: { bg: 'bg-purple-50', border: 'border-purple-200', tag: 'text-purple-600', tagBg: 'bg-purple-100' },
  route_teal:   { bg: 'bg-teal-50',   border: 'border-teal-200',   tag: 'text-teal-600',   tagBg: 'bg-teal-100' },
  route_orange: { bg: 'bg-orange-50',  border: 'border-orange-200', tag: 'text-orange-600',  tagBg: 'bg-orange-100' },
};
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {busEntries.map(([busId, bus]) => {
        ...
        return (
          <div key={busId} className="card-panel p-3 flex flex-col gap-2">
            {/* Header row */}
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[9px]">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold capitalize" style={{ color: trafColor }}>
